...
echo 'Adding module directory to proxy_http CMakeLists.txt'
sed -i '/# Subdirectories/a add_subdirectory\('$MODULE_DIR_NAME'\)' CMakeLists.txt

#Add module to list of libraries to link
echo 'Adding module as proxy_http dependency'
sed -i '/# Libraries/a target_link_libraries\(proxy_http '$MODULE_NAME'\)' CMakeLists.txt
