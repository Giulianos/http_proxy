...
typedef enum { UNDEFINED, V_1_0, V_1_1 } httpVersion;
...
uint8_t readAndWriteWithZero(buffer* b, buffer* bOut, bool* bEmpty);
...
* en sentido inverso (ideal para escribir en zona reservada).
*/
void writeDecToBufReverse(int number, buffer* b);

/** Escribo un número en b pasado a formato hexadecimal y
* en sentido inverso (ideal para escribir en zona reservada).
*/
void writeHexToBufReverse(int number, buffer* b);
 ...
 */
bool writeToTransfBufWithZero(buffer* b, buffer* bOut, int* quantity,
                              bool* bEmpty);

/** Como writeToBufReverse pero sin especificar la cantidad de elementos. */
void writePrefix(buffer* b, char* prefix);

/** Comparo lo siguiente a leer de b con un formato dado y lo paso a bOut a
* medida que leo.
* Si no terminé la comparación y b queda vacío, pongo bEmpty en true y pongo
* todo lo que comparé
* hasta el momento en la zona reservada de b. Esto es para que la siguiente vez
* que convoque matchFormat
* tenga con que comparar y como lo anterior está en la zona reservada esto no se
* vuelve a pasar a bOut.
* Adicionalmente para cuando el buffer queda vació, puedo escribir un prefijo en
* la zona reservada.
* Esto resulta útil para cuando hubo operaciones antes del matchFormat que voy a
* tener que realizar al
...
bool simpleMatchFormat(char* format, buffer* b, char* prefix, bool* bEmpty);

/** Leo un número de b en formato decimal y lo paso a bOut.
* Si b queda vacío durante la operación, hago algo similar a lo indicado para
* matchFormat.
*/
...
* matchFormat.
*/
bool getHexNumber(int* number, buffer* b, buffer* bOut, char* prefix,
                  bool* bEmpty);

/** Como getHexNumber pero sin escribir en un buffer de salida. */
bool simpleGetHexNumber(int* number, buffer* b, char* prefix, bool* bEmpty);
...
bool simpleCheckCRLF(buffer* b, char* prefix, bool* bEmpty);
