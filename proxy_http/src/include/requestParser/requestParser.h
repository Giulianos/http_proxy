...
typedef enum {
  ...
  GENERAL_ERROR,
  VERSION_ERROR,
  START_LINE_END_ERROR,
  GENERAL_METHOD_ERROR,
...
/** Seteo la estructura con los valores por default previos a la primer
 * utilización del parser. */
void defaultRequestStruct(RequestData* rData);

/** Función que llamo para correr el parser por primera vez. Internamente tiene
* una función
* a la cual puedo despertar cada vez que tengo algo en el buffer de entrada
* hasta encontrar al host.
*/
bool checkRequest(RequestData* rd, buffer* bIn, buffer* bOut,
                  void (*hostCallback)(const char*, int, void*),
                  void* callbackData);
