...
void listen_read_handler(struct selector_key* key);

int initialize_configurations(const char* media_types, const char* cmd);
...
main(const int argc, const char** argv)
{
  ...
  fd_handler logging_handlers = {.handle_read = NULL,
  ...
  ss = selector_register(selector, admin_socket, &admin_handler, OP_READ,
                         &admin_data);
...
listen_read_handler(struct selector_key* key)
{
  ...
  struct client_config config = {
    .selector = key->s, .fd = client_socket,
  };
