...
send_msg(addr_data_t servdata, int socket, msg_t* msg)
{
  ...
  return sctp_sendmsg(socket, msg, pointer - buffer, servdata->addr,
                      servdata->len, 0, 0, servdata->sri.sinfo_stream, 0, 0);
}
...
rcv_msg(addr_data_t servdata, int socket, msg_t* msg)
{
  ...
  read_quan =
    sctp_recvmsg(socket, buffer, sizeof(buffer), (struct sockaddr*)&peer, &len,
                 &servdata->sri, &msg_flags);
