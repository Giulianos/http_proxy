...
int
log_start(struct log* l, fd_selector s)
{
  int fd[2];
  if (pipe(fd) == -1)
    return PIPE_FAIL;
  ...
  pthread_create(&l->logthread, NULL, (void* (*)(void*)) & log_thread,
                 (void*)l);
  return LOG_OK;
}

void
log_thread(struct log* l)
{
  ...
  while (true) {
    ...
    if ((position < 2046 && c == 0) || position == 2046) {
      syslog(LOG_INFO, "%s", (const char*)cache);
      memset(cache, 0, 2048);
      position = 0;
    }
  }
...
void
log_sendf(struct log* l, const char* fmt, ...)
{
  char buffer[2048];
  va_list args;
  va_start(args, fmt);
...
void
log_close(struct selector_key* key)
{
  struct log* l;
  l = GET_LOG(key);
