...
static void assertReadAndWriteWithZero(buffer* b, buffer* bOut);
...
static void assertWriteToTransfBufWithZero(buffer* b, buffer* bOut);
...
static void assertWriteHexToBufReverse(buffer* b);
...
assertReadAndWriteWithZero(buffer* b, buffer* bOut)
{
  ...
  assert(readAndWriteWithZero(b, bOut, &bEmpty) ==
         'H'); // Elementos zona reservada
  assert(readAndWriteWithZero(b, bOut, &bEmpty) == 0);
  assert(readAndWriteWithZero(b, bOut, &bEmpty) ==
         'O'); // Elementos zona normal
  assert(buffer_read(bOut) == 'O');
  assert(readAndWriteWithZero(b, bOut, &bEmpty) == 0);
  assert(buffer_read(bOut) == 0);
...
assertWriteHexToBufReverse(buffer* b)
{
  ...
  assert(buffer_read(b) ==
         0); // Verifico que el buffer inicialmente esté vacío.
