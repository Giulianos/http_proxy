...
checkRequestInner(RequestData* rData, buffer* bIn, buffer* bOut)
{
  ...
  if (!success && !rData->isBufferEmpty) {
    ...
    switch (rData->parserState) {
      ...
      case METHOD:
        rData->state =
          (rData->method == UNDEFINED_M ? GENERAL_METHOD_ERROR
...
{
  char* methodOption[] = { "CONNECT", "DELETE", "GET", "HEAD",
                           "OPTIONS", "POST",   "PUT", "TRACE" };
  httpMethod methodType[] = { CONNECT, DELETE, GET, HEAD,
                              OPTIONS, POST,   PUT, TRACE };
...
cleanRelativeUri(RequestData* rData, buffer* bIn, buffer* bOut)
{
  ...
  // El relative uri no es de interés para el parser por lo que solo busco
  // pasarlo de largo.
...
checkUriForHost(RequestData* rData, buffer* bIn, buffer* bOut)
{
  ...
  // Si llego a este punto es porque tengu un uri absoluto que empieza con
  // http:// o https://.
  ...
  if (c == ':') {
    readAndWrite(bIn, bOut);
    validHost =
      getNumber(&(rData->port), bIn, bOut, ":", &(rData->isBufferEmpty));
  }
...
checkLocalHost(RequestData* rData, buffer* bIn, buffer* bOut)
{
  ...
  } else if (matchFormat("X-LOCALHOST:", bIn, bOut, "",
                         &(rData->isBufferEmpty))) {
...
extractHost(RequestData* rData, buffer* bIn, buffer* bOut)
{
  ...
  c = buffer_peek(
    bIn); // Por las validaciones anteriores ya se que voy a tener algo != 0.
