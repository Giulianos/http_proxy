...
char* error_msg_to_string(enum error_type type);
...
send_msg(addr_data_t servdata, int socket, msg_t* msg)
{
  ...
  sent_bytes =
    sctp_sendmsg(socket, buffer, pointer - buffer, servdata->addr,
                 servdata->addr_len, 0, 0, servdata->sri->sinfo_stream, 0, 0);
  if (sent_bytes <= 0) {
    printf("%s\n", strerror(errno));
  }
...
rcv_msg(addr_data_t servdata, int socket, msg_t* msg)
{
  ...
  read_quan =
    sctp_recvmsg(socket, buffer, MAX_READ, (struct sockaddr*)servdata->peer,
